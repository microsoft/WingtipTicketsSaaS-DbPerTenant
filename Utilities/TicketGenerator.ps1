...
{       
    ...
    if ($eventSections)
    {
        ...
        Write-Output "Inserting $tpId TicketPurchases for $($db.DatabaseName)"  
        
        $tpCommand = $tpCommand.TrimEnd((" ",",","`n")) + ";"

        $tpCommand += "SET IDENTITY_INSERT [dbo].[TicketPurchases] OFF"

        $ticketPurchasesExec = Invoke-Sqlcmd -Username "$AdminUserName" -Password "$AdminPassword" -ServerInstance $fullyQualifiedServerName -Database $db.DatabaseName -Query $tpCommand -ConnectionTimeout 30 -QueryTimeout 120 -EncryptConnection

        Write-Output "Inserting $tpId Tickets for $($db.DatabaseName)" 
               
        $tCommand = $tCommand.TrimEnd((" ",",","`n")) + ";"

        $ticketsExec = Invoke-Sqlcmd -Username "$AdminUserName" -Password "$AdminPassword" -ServerInstance $fullyQualifiedServerName -Database $db.DatabaseName -Query $tCommand -ConnectionTimeout 30 -QueryTimeout 120 -EncryptConnection
    }       
    else
    {
